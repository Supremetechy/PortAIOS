...
async def main():
    ...
    async with websockets.serve(
        ...
        # Open the WS client page via the Eel HTTP server (port 8001).
        # A 2-second delay lets Eel finish binding before the browser request lands.
        def _open_client():
            import time
            time.sleep(2)
            webbrowser.open("http://localhost:8001/avatar-ws-client.html")
        threading.Thread(target=_open_client, daemon=True).start()
